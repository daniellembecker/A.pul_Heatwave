...
# Plot
pdf("../output/pi_curves/PI_curves_calc_nls.pdf")
curves %>% 
  ...
  labs(x = expression(paste('PAR (', mu, "mol photons m"^-2, 's'^-1,")")),
       ...
       title = "Heatwave PI Curves: March 2022")
dev.off()
...
mean(meanpars$Ik) #645.2079

# Save mean photosynthesis pars for each colony to file
meanpars %>%
  mutate(experiment="heatwave") %>%
  write_csv(path = "../output/pi_curves/photosynthesis_spars_summary_nls.csv")
```
