...
data.filtered <-data %>%
  filter(!(Date.Time < "2022-03-20 23:59:00")) %>%
...
dates_vline <- as_datetime(c("2022-03-21 09:00:00", "2022-03-26 09:00:00", "2022-03-31 09:00:00", "2022-04-10 09:00:00", "2022-04-18 09:00:00"))               

#Define positions of vline

#dates_vline <- which(data.subset.mean.temp$Date.Time %in% dates_vline)
 
#data.subset.mean.temp <- separate(data.subset.mean.temp, da, c("date", "time"), sep = " ")


#colnames(data.subset.mean.temp)[1] <- "Date"
#data.subset.mean.temp$Date = as.Date(data.subset.mean.temp$Date.Time, format='%Y-%m-%d %H:%M:%S')

#tried to make vetor to add onto scale_x_dates
#dates <- c("2022-03-21 09:00:00", "2022-03-26 09:00:00", "2022-03-31 09:00:00", "2022-04-10 09:00:00", "2022-04-18 09:00:00")
#dates <- ymd_hms(dates)
...
temps <-data.subset.mean.temp %>%
  ...
  geom_line() + 
  geom_point() +
  #scale_x_datetime(date_breaks = c("2022-03-21 09:00:00", "2022-03-26 09:00:00", "2022-03-31 09:00:00", "2022-04-10 09:00:00", "2022-04-18 09:00:00", date_labels = "%y-%m-%d %h-%m-%s")) +
  #scale_x_date(breaks = as_Date(c("2022-03-21 09:00:00", "2022-03-26 09:00:00", "2022-03-31 09:00:00", "2022-04-10 09:00:00", "2022-04-18 09:00:00"))) +
  #scale_x_date(date_labels="%y-%m-%d %h-%m-%s", breaks = as.Date(c("2022-03-21 09:00:00", "2022-03-26 09:00:00", "2022-03-31 09:00:00", "2022-04-10 09:00:00", "2022-04-18 09:00:00"))) +
  scale_x_datetime(date_breaks = "week", date_labels = "%m-%d") +
  ...
  xlab("Date")+
  geom_vline(xintercept=ymd("2022-03-21 10:00:00"),color="red") +
  theme(axis.title=element_text(face="bold", size=60),
    ...
    legend.text=element_text(size=60)) + guides(color= guide_legend(override.aes = list(size = 6)));temps

temps <- temps + geom_vline(xintercept = as.numeric(dates_vline),
             col = "black", lwd = 0.5); temps

ggsave("output/HOBO_temp_light_tanks/All_Tank_HoboTemp.png", dpi=300, plot = temps, width = 49, height = 30, units="in")
ggsave("output/HOBO_temp_light_tanks/All_Tank_HoboTemp.pdf", dpi=300, plot = temps, width = 49, height = 30, units="in")
