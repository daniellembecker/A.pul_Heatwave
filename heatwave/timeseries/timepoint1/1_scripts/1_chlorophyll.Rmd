...
chl_path <- "timepoint1/1_data/1_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("timepoint1/1_data/1_homogenate_vols/1_homogenate_vols.csv") %>%
  select(fragment_ID, homogenate_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("timepoint1/1_output/1_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "timepoint1/1_output/1_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("timepoint1/1_metadata.csv")
