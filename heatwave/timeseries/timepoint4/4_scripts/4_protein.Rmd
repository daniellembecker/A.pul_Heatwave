---
title: "Heatwave TP4 host and holobiont protein analysis"
...
prot_path = "timepoint4/4_data/4_protein/4_host/"         # Path to prot data directory
...
#Error in nls(formula = BSA_ug.mL ~ z + a * exp(b * abs562), start = list(z = 0,  : step factor 0.000488281 reduced below 'minFactor' of 0.000976562, run linear model
...
```{r}
# Surface area data
sa <- read.csv("timepoint4/4_output/4_surface.area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("timepoint4/4_data/4_homogenate_vols/4_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("timepoint4/4_metadata.csv") %>% select(1:4)
...
prot_host %>%
  ...
  mutate(timepoint="TP4")%>%
  write_csv(., path = "timepoint4/4_output/4_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "timepoint4/4_data/4_protein/4_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
# Coral sample metadata
metadata <- read_csv("timepoint4/4_metadata.csv") %>% select(1:5)
...
prot_holobiont %>%
  ...
  select(fragment_ID, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="TP4")%>%
  write_csv(., path = "timepoint4/4_output/4_holobiont_protein.csv")
```
