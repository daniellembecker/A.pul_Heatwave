...
chl_path <- "timepoint4/4_data/4_chlorophyll_redo/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("timepoint4/4_data/4_homogenate_vols/4_homogenate_vols.csv") %>%
  select(fragment_ID, homogenate_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("timepoint4/4_output/4_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "timepoint4/4_output/4_chlorophyll_REDO.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("timepoint4/4_metadata.csv")

chl <- right_join(chl, meta, by = "fragment_ID")

# Plot chlorophyll a
chl %>%
  ggplot(aes(x = timepoint, y = chla.ug.cm2, color = treatment.x)) +
  #facet_wrap(~species) +
...
# Plot chlorophyll c2
chl %>%
  ggplot(aes(x = timepoint , y = chlc2.ug.cm2, color = treatment.x)) +
  #facet_wrap(~species) +
