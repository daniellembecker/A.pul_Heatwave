---
title: "Heatwave TP5 host and holobiont protein analysis"
author: "HM Putnam, AS Huffmyer"
edited by: "DM Becker-Polinski"
date: "20220306"
5_output: html_document
editor_options: 
  chunk_5_output_type: console
---
...
prot_path = "timepoint5/5_data/5_protein/5_host/"         # Path to prot data directory
...
df_host <- tibble(file = prot_data_files) %>%
  ...
  mutate(platemap = map(plate, ~read_csv(paste0(prot_path, ., "_platemap.csv"))),
...
```{r}
# Surface area data
sa <- read.csv("timepoint5/5_output/5_surface.area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("timepoint5/5_data/5_homogenate_vols/5_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("timepoint5/5_metadata.csv") %>% select(1:4)
...
# Write data to 5_output file
```{r}
# Write protein data to 5_output file
...
prot_host %>%
  ...
  mutate(timepoint="TP5")%>%
  write_csv(., path = "timepoint5/5_output/5_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "timepoint5/5_data/5_protein/5_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
#Error in nls(formula = BSA_ug.mL ~ z + a * exp(b * abs562), start = list(z = 0,  : singular gradient, run linear model
...
# Coral sample metadata
metadata <- read_csv("timepoint5/5_metadata.csv") %>% select(1:5)
...
prot_holobiont %>%
  ...
  select(fragment_ID, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="TP5")%>%
  write_csv(., path = "timepoint5/5_output/5_holobiont_protein.csv")
```
