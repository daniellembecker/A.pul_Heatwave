...
## Import data
```{r}
path.p <- "timepoint5/5_data/5_PR/5_PR_files/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "timepoint5/5_data/5_PR/5_PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "timepoint5/5_data/5_PR/5_PR_run_metadata.csv")
...
#edit fragment_ID name to remove .csv and change _TP1 to _2
df <- tibble(file.name = file.names) %>%
  ...
  mutate(fragment_ID = gsub("_TP5", "_5", fragment_ID),                              # remove TP1 and replace with 1
...
sa <- read_csv(file = "timepoint5/5_output/5_surface.area.calc.csv")
...
# Write to output file
write.csv(pr.out, "timepoint5/5_output/5_PR/5_PR_rates.csv")
```
...
ggsave("timepoint5/5_output/5_PR/5_Timepoint.Treatment_PR.pdf", plot, width = 6, height = 5, units = c("in"))
