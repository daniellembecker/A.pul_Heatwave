...
chl_path <- "timepoint5/5_data/5_chlorophyll_redo/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("timepoint5/5_data/5_homogenate_vols/5_homogenate_vols.csv") %>%
  select(fragment_ID, homogenate_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("timepoint5/5_output/5_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "timepoint5/5_output/5_chlorophyll_REDO.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("timepoint5/5_metadata.csv")
