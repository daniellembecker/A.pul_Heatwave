...
```{r}
# Cell count data
sym_counts <- read_csv("timeseries/timepoint1/1_data/1_sym_counts/1_symbiont_counts.csv")

# Surface area data
sa <- read.csv("timeseries/timepoint1/1_output/1_surface.area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("timeseries/timepoint1/1_data/1_homogenate_vols/1_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("timeseries/timepoint1/1_metadata.csv") %>% select(1:4)
...
sym_counts %>%
  ...
  write_csv(path = "timeseries/timepoint1/1_output/1_sym_densities.csv")
