...
```{r}
biomass_files<-list.files("../", pattern = "biomass.calc.csv", recursive=T, full.names=T)
...
holobiont_protein_dataset <- holobiont_protein_dataset %>%
  rename(holobiont_prot_ug.cm2 = holobiont_prot_ug.cm2)
...
master <- full_join(full_join(full_join(full_join(
 ...
 # biomass_dataset, by=c("fragment_ID", "timepoint")),
...
master.1<-master%>%
 ...
 mutate(mgafdw = Host_AFDW.mg.cm2*surface.area.cm2)%>% #get absolute values of normaliers for each coral sample
  mutate(mgafdw = Sym_AFDW.mg.cm2*surface.area.cm2)%>% #get absolute values of normaliers for each coral sample
...
summary_fragments<-metadata_master%>%
  ...
  drop_na("timepoint")%>%
  write_csv(., "timeseries_analysis/Output/summary_physio_fragment.csv")
