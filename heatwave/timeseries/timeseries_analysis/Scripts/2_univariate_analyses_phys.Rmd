...
means_per_treatment <- master.filtered %>%
  group_by(treatment) %>%
  summarise(mean_sym_bio = mean(Sym_AFDW.mg.cm2))

means_per_treatment

```

Interaction of timepoint and treatment and main effects are significant.
...
means_per_treatment <- master.filtered %>%
  group_by(treatment) %>%
  summarise(mean_host_bio = mean(Host_AFDW.mg.cm2))
