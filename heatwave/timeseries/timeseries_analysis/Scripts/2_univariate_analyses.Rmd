...
letters_data_sym <- as.data.frame(cld_result_sym)
...
# run post hoc on timepoints
emm<-emmeans(chla_model, ~ timepoint)
pairs(emm)
...
# run post hoc on timepoints
emm<-emmeans(chlc_model, ~ timepoint)
pairs(emm)
...
letters_data_sym <- letters_data_sym %>%
  select(timepoint, .group)

# Prepare your plotting data
plot_data_sym <- master %>%
  filter(!is.na(cells.cm2)) %>%
  filter(!is.na(timepoint)) %>%
  select(fragment_ID, timepoint, formatted_date, treatment, cells.cm2)

# Merge the plotting data with the letters
plot_data_sym <- merge(plot_data_sym, letters_data_sym, by = "timepoint")
...
cld_summary_sym <- plot_data_sym %>%
  group_by(timepoint, formatted_date) %>%
...
#  geom_text(data = cld_summary_sym, aes(x = formatted_date, y = max_y + 0.1 * max_y, label = .group), size = 6, vjust = -0.5) + # Add letters
...
holobiont_prot_cm2<-master.holo.prot %>%
  ...
  ggplot(., aes(x = formatted_date, y = holobiont_prot_mg.cm2, fill = treatment)) +
    geom_boxplot(outlier.size = 0) +
    geom_point(pch = 21, size=2, position = position_jitterdodge(0.2)) + 
    scale_fill_manual(values = c("darkblue", "red", "gray"), labels=c("ambient", "hot", "initial"))+
    labs(fill = "Treatment") +
...
tot_chl <- filtered_master.tot.chl %>%
  ...
  ggplot(., aes(x = formatted_date, y = Total_Chl_cm2, fill = treatment)) +
    geom_boxplot(outlier.size = 0) +
    geom_point(pch = 21, size=2, position = position_jitterdodge(0.2)) + 
    scale_fill_manual(values = c("darkblue", "red", "gray"), labels=c("ambient", "hot", "initial"))+
    labs(fill = "Treatment") +
