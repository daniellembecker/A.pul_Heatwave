---
title: "Heatwave TP3 host and holobiont protein analysis"
...
prot_path = "timepoint3/3_data/3_protein/3_host/"                                              # Path to prot data directory
...
homog_vols <- read_csv("timepoint3/3_data/3_homogenate_vols/3_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("timepoint3/3_metadata.csv") %>% select(1:4)
...
prot_host %>%
  ...
  write_csv(., path = "timepoint3/3_output/3_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "timepoint3/3_data/3_protein/3_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
# Coral sample metadata
metadata <- read_csv("timepoint3/3_metadata.csv") %>% select(1:5)
...
prot_holobiont %>%
  ...
  select(fragment_ID, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="TP3")%>%
  write_csv(., path = "timepoint3/3_output/3_holobiont_protein.csv")
```
