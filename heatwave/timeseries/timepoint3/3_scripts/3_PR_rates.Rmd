...
## Import data
```{r}
path.p <- "timepoint3/3_data/3_PR/3_PR_files/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "timepoint3/3_data/3_PR/3_PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "timepoint3/3_data/3_PR/3_PR_run_metadata.csv")
...
df <- tibble(file.name = file.names) %>%
  ...
  mutate(fragment_ID = gsub("_TP3", "_3", fragment_ID),                              # remove TP1 and replace with 1
...
sa <- read_csv(file = "timepoint3/3_output/3_surface.area.calc.csv")
...
# Write to output file
write.csv(pr.out, "timepoint3/3_output/3_PR/3_PR_rates.csv")
```
...
ggsave("timepoint3/3_output/3_PR/3_Timepoint.Treatment_PR.pdf", plot, width = 6, height = 5, units = c("in"))
