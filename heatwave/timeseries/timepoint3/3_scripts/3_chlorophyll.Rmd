...
chl_path <- "timepoint3/3_data/3_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("timepoint3/3_data/3_homogenate_vols/3_homogenate_vols.csv") %>%
  select(fragment_ID, homogenate_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("timepoint3/3_output/3_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "timepoint3/3_output/3_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("timepoint3/3_metadata.csv")
