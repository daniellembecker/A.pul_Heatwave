...
prot_path = "timepoint2/2_data/2_protein/2_host/"                                              # Path to prot data directory
...
prot_host %>%
  ...
  write_csv(., path = "timepoint2/2_output/2_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "timepoint2/2_data/2_protein/2_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
# Coral sample metadata
metadata <- read_csv("timepoint2/2_metadata.csv") %>% select(1:5)
...
prot_holobiont %>%
  ...
  select(fragment_ID, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="TP2")%>%
  write_csv(., path = "timepoint2/2_output/2_holobiont_protein.csv")
```
