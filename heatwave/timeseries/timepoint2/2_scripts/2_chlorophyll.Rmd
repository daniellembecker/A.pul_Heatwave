...
chl_path <- "timepoint2/2_data/2_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("timepoint2/2_data/2_homogenate_vols/2_homogenate_vols.csv") %>%
  select(fragment_ID, homogenate_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("timepoint2/2_output/2_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "timepoint2/2_output/2_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("timepoint2/2_metadata.csv")
