...
## Import data
```{r}
path.p <- "timepoint2/2_data/2_PR/2_PR_files/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "timepoint2/2_data/2_PR/2_PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "timepoint2/2_data/2_PR/2_PR_run_metadata.csv")
...
df <- tibble(file.name = file.names) %>%
  ...
  mutate(fragment_ID = gsub("_TP2", "_2", fragment_ID),                              # remove TP1 and replace with 1
...
sa <- read_csv(file = "timepoint2/2_output/2_surface.area.calc.csv")
...
# Write to output file
write.csv(pr.out, "timepoint2/2_output/2_PR/2_PR_rates.csv")
```
...
ggsave("timepoint2/2_output/2_PR/2_Timepoint.Treatment_PR.pdf", plot, width = 6, height = 5, units = c("in"))
