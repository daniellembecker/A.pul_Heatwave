...
```{r, echo=FALSE}
# Read the Trinotate report, assuming it's tab-separated and make annot_GO format by pulling out go terms
...
# Save on desktop because too large for github
write.csv(combined_go_terms, "../../A.pul_transcriptome_functional_annotation.csv", row.names = FALSE, quote = FALSE)

#Read in organized annotation file
annot.GO <- read.csv("../../A.pul_transcriptome_functional_annotation.csv")
```
...
#load DEG results for adults
DEG.sig <- read.csv("bioinformatics/output/adult_deg_")[,-1]
# Rename 'gene' column to 'gene_id'
colnames(DEG.sig)[colnames(DEG.sig) == "gene"] <- "gene_id"
nrow(DEG.sig)

# Load filtered gene count file for adults
gcount_filt <- read.csv("bioinformatics/output/gcount_filt_adult.csv", header = TRUE, sep = ",")
...
gcount_filt2annot = match(gcount_filt$gene_id, annot.GO$gene_id) #match genes in gcount_filt to annot_GO
...
```{r}
filtered_Apul.annot <- annot.GO[annot.GO$gene_id %in% gcount_filt$gene_id, ]
dim(filtered_Apul.annot)

# Remove leading semicolon before the first GO term
filtered_Apul.annot$GO.ID <- gsub("^;\\s+", "", filtered_Apul.annot$GO.ID)
```
