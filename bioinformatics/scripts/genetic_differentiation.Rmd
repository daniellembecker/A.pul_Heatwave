...
# Create population factor
pop.larvae <- factor(sample_data.larvae$treatment)
...
# Add treatment and genotype
pca_df.larvae$treatment_genotype <- c("Ambient_5", "Ambient_8", "Ambient_12", "Heatwave_4", "Heatwave_8", "Heatwave_12")

# Plot the PCA
pca_plot.larvae <- ggplot(pca_df.larvae, aes(x = PC1, y = PC2, label = treatment_genotype)) +
  geom_point(size = 3) +
  geom_text(vjust = -1, hjust = 0.5) + 
