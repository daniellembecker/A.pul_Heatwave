...
```{r}
#DEG results
DEG.res <- read.csv("bioinformatics/output/adult_deg_sig.csv")[-1]
...
gcount_filt <- read.csv("bioinformatics/output/gcount_filt_adult.csv")
...
#print(missing)
```
5,681 genes in gcount_filt do not have GO annotations
...
```
The annotation file now only contains genes that were detected in our dataset that have annotation information. This is 13,476 out of the 72,533 genes in our dataset. 
...
png_file <- "bioinformatics/output/adult_DEG_top10_enrich_BP_table.png"
...
# Save flextable as PNG
png_file <- "bioinformatics/output/adult_DEG_top10_enrich_MF_table.png"
save_as_image(flex_enrich, path = png_file)
...
# Save flextable as PNG
png_file <- "bioinformatics/output/adult_DEG_top10_enrich_CC_table.png"
save_as_image(flex_enrich, path = png_file)
```

# Make plot with significant go terms
```{r}
adult.GO <- read.csv("bioinformatics/output/adult_GO_enrichment_terms.csv")

# Calculate -log10(classicFisher) and order the data
go_data_adult <- adult.GO %>%
...
# Create the ggplot
ggplot(go_data_adult, aes(x = neg_log10_fisher, y = Term, fill = ontology)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c( "Molecular Functions" = "lightgreen", 
  ...
  ggtitle("DEG Processes Enriched in Parental Heatwave Treatment")

# Save the plot
ggplot2::ggsave("bioinformatics/output/adult_GO_Terms_by_Ontology_Fisher.pdf", height = 28, width = 25)
ggplot2::ggsave("bioinformatics/output/adult_GO_Terms_by_Ontology_Fisher.png", height = 28, width = 25)
