...
flex_enrich <- flex_enrich %>%
  set_header_labels(
    ...
    Significant = "Significant",
classicFisher = "Fisher p-value") %>%
  colformat_double(j = "classicFisher", digits = 4) %>%
  compose(j = "classicFisher", value = as_paragraph(
    as_chunk(~ format_pvalue(classicFisher))
  )) %>%
  bold(j = "classicFisher", i = ~ as.numeric(classicFisher) < 0.05) %>%
