...
annot_GO <- read.csv("../../A.pul_transcriptome_functional_annotation.csv")
...
```{r}
#DEG results
DEG.res <- read.csv("bioinformatics/output/larvae_deg_sig.csv")[-1]
...
fisher.up_BP <- runTest(GOdata.up_BP, algorithum = "weight", statistic = "fisher")
...
# Tells you how many signifcant genes there was in the analysis 
numSigGenes(GOdata.down_MF) #104
```
...
# Tells you how many signifcant genes there was in the analysis 
numSigGenes(GOdata.down_CC) #757
```
...
# Generate a results table
allRes_up_CC_10 <- GenTable(GOdata.up_CC, 
                      classicFisher = fisher.up_CC, 
                      ranksOf = "classicFisher", 
                      topNodes = 10)
allRes_up_CC_10$Direction <- "Up-Regulated"

# Generate a results table
allRes_down_CC_10 <- GenTable(GOdata.down_CC, 
                      classicFisher = fisher.down_CC, 
                      ranksOf = "classicFisher", 
                      topNodes = 10)
allRes_down_CC_10$Direction <- "Down-Regulated"
...
flex_enrich <- flex_enrich %>%
  ...
  bold(j = "classicFisher", i = ~ classicFisher < 0.05) %>%  # Bold only values < 0.05 in classicFisher column
  add_header_lines(values = "Top 10 Enriched GO Terms (Biological Process)") %>%
  autofit() %>%
  theme_box()

# Save flextable as PNG
png_file <- "bioinformatics/output/larval_DEG_top10_enrich_BP_table.png"
...
# Save flextable as PNG
png_file <- "bioinformatics/output/larval_DEG_top10_enrich_MF_table.png"
save_as_image(flex_enrich, path = png_file)

# Combine top results for down and upreagulated MF
Enrichment_Res_CC_10 <- rbind(allRes_up_CC_10,allRes_down_CC_10)

# Create publishable table
Enrichment_Res_CC_10_table <- Enrichment_Res_CC_10 %>%
 dplyr::select(-Expected, -Direction)  # Remove the 'expected' and 'direction" column

# Create flextable object
flex_enrich <- flextable(Enrichment_Res_CC_10_table)
...
classicFisher = "Fisher p-value") %>%
  colformat_double(digits = 4) %>%
  bold(j = "classicFisher", i = ~ as.numeric(classicFisher) < 0.05) %>%  # Bold values < 0.05, including scientific notation
  add_header_lines(values = "Top 10 Enriched GO Terms (Cellular Components)") %>%
  autofit() %>%
  theme_box()

# Save flextable as PNG
png_file <- "bioinformatics/output/larval_DEG_top10_enrich_CC_table.png"
save_as_image(flex_enrich, path = png_file)
```


# Make plot with significant go terms
```{r}
larval.GO <- read.csv("bioinformatics/output/larval_GO_enrichment_terms.csv")

# Load required libraries
library(ggplot2)

# Convert 'classicFisher' p-values to -log10 scale for better visualization
larval.GO$log10Fisher <- -log10(larval.GO$classicFisher)

# Calculate gene count: the sum of significant genes assigned to a specific GO term
gene_counts <- larval.GO %>%
  group_by(Term) %>%
  summarise(GeneCount = sum(Significant)) %>%
  ungroup()

# Merge the gene counts back to the original dataframe
larval.GO <- larval.GO %>%
  left_join(gene_counts, by = "Term")

# Calculate the gene ratio (Significant / Annotated)
larval.GO <- larval.GO %>%
  mutate(GeneRatio = Significant / Annotated)

# Order GO Terms by GeneCount (decreasing order) for better visualization
larval.GO$Term <- factor(larval.GO$Term, levels = larval.GO$Term[order(larval.GO$GeneCount)])

# Ensure 'ontology' is a factor for proper plotting
larval.GO$ontology <- factor(larval.GO$ontology, levels = c("BP", "MF", "CC"))

# Create the plot
ggplot(larval.GO, aes(x = GeneRatio, y = Term, size = GeneCount, fill = log10Fisher)) +
  geom_point(shape = 21) +

  # Set the size range for points and the color gradient for significance levels
  scale_size(range = c(2.5, 12.5)) +
  scale_fill_continuous(low = 'royalblue', high = 'red4') +

  # Labels for axes and plot
  xlab('Gene Ratio (Significant / Annotated)') + ylab('GO Term') +
  labs(
    subtitle = 'Larval Significant GO terms') +

  # Customize the theme for better presentation
  ...
  # Facet by ontology with wrapping
  #facet_wrap(~ ontology, scales = 'free', ncol = 1) +

  # Optionally flip coordinates if preferred
 # coord_flip()

# Save the plot
ggplot2::ggsave("bioinformatics/output/larval_GO_Terms_by_Ontology_Fisher.pdf", height = 12, width = 12)
