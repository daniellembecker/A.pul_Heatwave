...
# Calculate -log10(classicFisher) and order the data
go_data_larvae <- larval.GO %>%
  mutate(neg_log10_fisher = -log10(classicFisher)) %>%
  arrange(ontology, desc(neg_log10_fisher)) %>%
  group_by(ontology) %>%
...
# Create the ggplot
ggplot(go_data_larvae, aes(x = neg_log10_fisher, y = Term, fill = ontology)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("Biological Processes" = "lightblue", 
                               "Molecular Function" = "lightgreen", 
                               "Cellular Components" = "lightcoral")) +
  labs(x = "-log10(classic Fisher)", y = "Terms", fill = "Ontology") +
  ...
  theme(axis.text.y = element_text(size = 38),
        ...
        plot.title = element_text(hjust = 0.5, size = 58),
        strip.text = element_text(angle = 0, size = 30),
        strip.placement = "outside",
        strip.background = element_rect(fill = "white"),
        panel.spacing = unit(1, "lines")) +
  ggtitle("DEG Processes Enriched in Larval Heatwave Parental History")

# Save the plot
ggplot2::ggsave("bioinformatics/output/larval_GO_Terms_by_Ontology_Fisher.pdf", height = 33, width = 28)
ggplot2::ggsave("bioinformatics/output/larval_GO_Terms_by_Ontology_Fisher.png", height = 33, width = 30)
