...
size_plot<-size %>%
  ...
  xlab(expression(bold(paste("Treatment"))))+
    ...
    ggtitle("Kruskal-Wallis, p < 0.0001") +
      annotate("text", x = 1.45, y = 0.220, label = "*", size = 20) +
    theme_classic() + 
...
size_plot_mean<-size %>%
  ...
  summarise(mean=mean(volume, na.rm=TRUE), 
   ...
   xlab(expression(bold(paste("Treatment"))))+
    theme_classic() + 
    annotate("text", x = 1.45, y = 0.110, label = "*", size = 20) +
    ylim(0,0.12)+
    theme(plot.title = element_text(hjust = 0.5, vjust = -0.1),
      legend.position="none",
      axis.title=element_text(face="bold", size=20, color="black"),
...
total_fert_success_plot <- calculate_fert_success %>%
  ...
  xlab(expression(bold(paste("Treatment"))))+
    ...
    ggtitle("Kruskal-Wallis, p = 0.04") +
      annotate("text", x = 1.45, y = 18, label = "*", size = 20) +
    theme_classic() + 
    theme(plot.title = element_text(hjust = 0.5, vjust = -0.1),
      legend.position="none",
      axis.title=element_text(face="bold", size=20, color="black"),
...
ggsave("output/reproductive_ecology/all_repro.png", plot = repro_plot, width = 15, height = 8)
ggsave("output/reproductive_ecology/all_repro.pdf", plot = repro_plot, width = 20, height = 12)
